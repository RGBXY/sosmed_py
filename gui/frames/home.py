...
class HomeFrame(tk.Frame):
    """Frame halaman utama (Home Feed) tempat pengguna melihat, membuat, mengedit, dan menghapus postingan."""

    def __init__(self, parent, current_user, data_edit=None):
        """Menginisialisasi session user, membangun tata letak grid feed, dan mempersiapkan area scrollable."""
        ...
        # Setup Sidebar Kiri Utama
        render_role_sidebar(self, current_user, "Home")

        # Container Kanan Utama (Menampung Header + Feed + Widget Kanan)
        ...
        # Setup Scrollbar untuk Feed Postingan
    ...
    def load_data(self):
        """Mengambil data records postingan terbaru dari layer business logic untuk dipetakan ke komponen visual."""
        post_data = self.post.get_posts_logic(self.current_user.id)
        
        for post in post_data:
            post.type = 'post'
            
        self.render_posts(post_data)

    def update_to_edit_mode(self, post_data):
        """Mengubah status form menjadi mode edit dengan menyuntikkan data lama dan mengarahkan scrollbar ke atas."""
        self.post_data_edit = post_data
        self.render_post_form()
        self.canvas.yview_moveto(0)

    def handle_form_success(self):
        """Callback event pasca operasi form berhasil untuk mengosongkan status edit dan memuat ulang data feed."""
        self.post_data_edit = None 
        self.render_post_form()
        self.load_data()         

    def render_post_form(self):
        """Membersihkan area form container lama dan merender ulang komponen form input pembuatan/revisi postingan."""
    ...
    def render_posts(self, post_data):
        """Menghancurkan seluruh widget kartu lama dan menyusun ulang jajaran CreatePostCard secara dinamis."""
        for item in self.container_posts.winfo_children():
            item.destroy()
            
        for obj in post_data:
            if getattr(obj, 'type', None) == 'post':
                CreatePostCard(
    ...
    def on_delete(self, id):
        """Menampilkan dialog konfirmasi dan mengeksekusi penghapusan data postingan berdasarkan id unik via backend."""
    ...
    def on_frame_configure(self, event):
        """Menghitung ulang dimensi region koordinat Canvas dan menyembunyikan scrollbar jika isi konten tidak melebihi batas window."""
    ...
    def on_canvas_configure(self, event):
        """Mengunci lebar window internal Canvas agar responsif mengikuti perubahan ukuran frame induk."""
        self.canvas.itemconfig(self.canvas_window, width=event.width)
    
    def on_mousewheel(self, event):
        """Menangkap input perputaran roda mouse (scroll) untuk menggerakkan koordinat yview Canvas secara vertikal."""
